...
- **连单风单独音 / れんたん風単独音 / RenTanFu**：一条音频录制多个彼此分开的单独音。例如 `_か_き_く_け_こ`。外观看起来接近连续录音，但每个音仍然分开录制。
- **连单术 / れんたんじゅつ / RenTanJutsu**：连续录制单独音，更容易保持音长和发声状态稳定，也方便切出部分 VC。推荐第一次录制 UTAU 音源的人使用。RTJ 不包含全部 VC 音素，如需完整 VC，请使用 CVVC。
- **VCV**：录制母音到下一个音的连续连接，衔接自然，但需要录制较多的音，原音设定量也很大。
- **CVVC**：录制完整的 CV 和 VC 音素，原音设定相对复杂，比较适合有一定 UTAU 音源制作经验的人。

RenTanFu 与 RenTanJutsu 使用相同的 **树月式 / 标准日语** 音素集与相关设置。

当前连续音支持 **6～8 mora**，CVVC 支持 **4～8 mora**。  
连续音的 6 / 7 mora 会合并因截短产生的补充音素，尽量减少额外录音行；补助音补充则保持与 8 mora 一致的固定结构。

CVVC 的句首 CV 可以选择 **按音节组排列** 或 **集中到最后**。集中到最后时，会按清音、浊音、拗音清音、拗音浊音分段排列。

CVVC 使用 7 mora 录音表时，如果仍搭配 8 mora BGM，录音总时间会比直接录制 8 mora 更长。建议直接录制 8 mora，或使用 6 mora BGM 搭配 6 mora 录音表。
